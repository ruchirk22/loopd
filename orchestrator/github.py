...
def build_pr_body(task: str, steps: List[dict], forecast: Optional[dict],
                  issue: Optional[dict], decisions: List[str], finished: bool,
    ...
    if isinstance(confidence, dict) and confidence.get("score") is not None:
        bar = "✓ meets the >75% bar" if confidence.get("meets_bar") else "below the >75% bar"
        lines.append(f"- **Delivery confidence:** {confidence['score']}% "
                     f"({confidence.get('band', '?')}) — {bar}.")
        cov = next((f for f in confidence.get("factors", [])
                    if f.get("key") == "coverage"), None)
        if cov and cov.get("note"):
...
def assemble_pr(repo, decisions: Optional[List[str]] = None) -> Optional[dict]:
    ...
    confidence = None
    cp = ad / "confidence.json"
    if cp.is_file():
        try:
            confidence = json.loads(cp.read_text())
        except (OSError, json.JSONDecodeError):
            confidence = None
    body = build_pr_body(task, steps, st.get("forecast"), marker, decisions or [],
                         bool(st.get("finished")), confidence=confidence)
